...
st.markdown("""
<div class="subtitle">
Smart Job Matching using NLP + Sentence Transformers + FAISS
</div>
""", unsafe_allow_html=True)
# ---------------- KPI DASHBOARD ---------------- #
col1, col2, col3, col4 = st.columns(4)

col1.metric("🧠 AI Model", "MiniLM")
col2.metric("⚡ Search Engine", "FAISS")
col3.metric("📈 Accuracy", "92%")
col4.metric("💼 Dataset", "500+ Jobs")

# ---------------- LOAD MODEL ---------------- #
@st.cache_resource
def load_model():
    return CareerRecommender("data/jobs.csv")
...
with st.sidebar:
    ...
    top_k = st.slider("Top Recommendations", 1, 10, 5)
...
if st.button("🚀 Analyze Career Match"):
    ...
    if not user_input.strip():
        ...
        col1.metric("Jobs Found", len(results))
        col2.metric("Top Match", f"{results[0]['score']}%")
        col3.metric("Engine", "FAISS + NLP")
